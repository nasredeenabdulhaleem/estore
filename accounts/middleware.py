    ...
    def __call__(self, request):
        ...
        if not request.path.startswith('/admin/') and request.path not in exclude_urls:
            if request.user.is_authenticated and request.user.role == 'Vendor':
                verified = VerificationCount.objects.filter(user=request.user,is_verified=True).exists()
                if not verified:
                    return redirect('account-verification')
                store_exists = VendorStore.objects.filter(vendor__user=request.user).exists()
                if not store_exists:
